...
const PickupInfo = ({ addOrder, cart, updateCart }) => {
  ...
  const handlePickUpOrder = () => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      console.log('Order placed:', customerOrderInfo);
      placeOrder();
  ...
  const handleContinue = async () => {
    ...
    const marker = localStorage.getItem('location');
    if (marker) {
      const { lat, lng } = JSON.parse(marker);
      form.location = {
        latitude: lat,
        longitude: lng,
        address,
  ...
  async function placeOrder() {
    ...
    try {
      if (form) {
        const res = await axios.post('https://recycle-backend-lflh.onrender.com/placeorder', form);
        if (res.status === 201) {
          alert('Order placed successfully');
          // Update pickupHistory
          const pickupHistoryData = { Id: form.Id, cart: form.cart };
          await axios.post('https://recycle-backend-lflh.onrender.com/api/updatePickupHistory', pickupHistoryData, {
            headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
          });
        }
      }
    ...
    <>
      {showLottie && (
        <div style={styles.lottieFullScreen}>
          <iframe 
            src="https://lottie.host/embed/53bb3da5-9633-4e4e-aa40-6bb8e175ad34/utGraX6uT3.json"
        ...
        <Billing>
          ...
          <StyledForm>
            <AddressContainer onClick={() => setShowModal(true)}>
              ...
              <AddressText>To: {address || "Select an address"}</AddressText>
            </AddressContainer>
        ...
        <Modal show={showModal} onHide={() => setShowModal(false)} centered>
          ...
          <Modal.Body>
            <SavedAddressList>
              ...
              >
                <SavedAddressCardContent>{addr}</SavedAddressCardContent>
              </SavedAddressCard>
              ))}
              <AddNewAddressCard onClick={() => navigate('/address-list')}>
                + Add New Address
              </AddNewAddressCard>
            </SavedAddressList>
            {errormessage.address && <Error>{errormessage.address}</Error>}
